...
export default function IconNavTwo() {
    ...
    <div className="hidden lg:flex items-center gap-4">
        {navIconList.map((i, key) => (
            <Link key={key} href={i.href} 
                ...
                justify-center rounded-full w-10 h-10 border border-gray-800 hover:border-amber-600`}>
                <IconDefault 
                    type={i.iconType} 
                    css="text-lg group-hover:scale-110 ease-in-out transition-all duration-200 group-hover:text-amber-500" /> 
            </Link>
        ))}   
    </div>
